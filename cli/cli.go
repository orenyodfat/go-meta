...
usage: meta import xml <file> [<context>...]
       ...
       meta server [--port=<port>] [--musicbrainz-index=<sqlite3-uri>] [--cwr-index=<sqlite3-uri>] [--eidr-index=<sqlite3-uri>]
...
func (cli *CLI) RunServer(ctx context.Context, args Args) error {
	...
	srv, err := NewServer(cli.store, musicbrainzDB, cwrDB, eidrDB)
	if err != nil {
		return err
	}
